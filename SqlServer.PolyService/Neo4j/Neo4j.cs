...
namespace SqlServer.PolyService
{
    ...
    public class Neo4j : RestWebService
    {
        public Neo4j()
        {
        ...
        {
            return RestWebService.Parse<Neo4j>(s);
        }
        ...
        public override bool IsEqual(RestWebService obj)
        {
            ...
            Neo4j nfj = (Neo4j)obj;
            if (this.Key != nfj.Key) return false;
        ...
        [SqlMethod(OnNullCall = false)]
        [return: SqlFacet(MaxSize = -1)]
        public SqlChars Query(SqlString query)
        {
        ...
        {
            FormatQuery();
            SqlChars ret = Query(query.ToString());
            ResetQuery();
            return ret;
        }
